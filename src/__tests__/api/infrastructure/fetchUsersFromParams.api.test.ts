...
describe("fetchUserFromApiWithParams", () => {
  it('fetches user details from API with correct ID', async () => {
    ...
    const result = await fetchUserFromApiWithParams(1);
  ...
  it('throws an error when the API call fails', async () => {
    ...
    await expect(fetchUserFromApiWithParams(1)).rejects.toThrow('Failed to fetch user with id: 1');
  });
})