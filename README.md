...
```bash
 npm install jest ts-jest @types/jest jest-mock-axios --save-dev
```
...
- To test an API use the `api` folder under `__tests__` folder, Inside the `api` folder there are two folders namely `infrastructure` and `application`
...
```
- This Way a test for an API would be written.